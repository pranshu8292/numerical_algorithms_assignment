...
class Interpolate:
    ...
    def plotgraph(self,value, xlower, xupper) :
        ...
        y=[]
        for x_val in x :
            y.append(P.polyval(x_val, value))
    ...
    def Lagrange(self,L,M,xlower,xupper):                                                
        ...
        for i in range(n):
            value=(P.polydiv(w,(-1*L[i],1))[0]*M[i])/P.polyval(L[i],derivative)
            self.plotgraph(value, xlower, xupper)
